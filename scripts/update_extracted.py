...
import polars as pl
from libraries import load_pci_friendly_set, mark_pci_friendly, format_table, norm_name, format_Scimago_Rank
import re
...
# Columns that should always be overwritten from Scimago when available
ALWAYS_OVERWRITE = {"Scimago Rank", "Scimago Quartile", "H index"}
...
def update_and_log_statistics(df: pl.DataFrame, totals: dict) -> pl.DataFrame:
    ...
    for col in COLUMNS_TO_UPDATE:
        ...
        # Default behavior: fill only when target is null
        updates = df.filter(pl.col(col).is_null() & pl.col(scimago_col).is_not_null()).height
        totals[col] += updates
        if updates > 0:
            print(f"  - '{col}' empty filled: {updates}")
        df = df.with_columns(
        ...
        if col in ALWAYS_OVERWRITE:
            # Always overwrite with the Scimago-derived value when available
            # Compare as Utf8 to avoid dtype mismatches during change counting
            changes = df.filter(
                pl.col(scimago_col).is_not_null()
                & (pl.col(col).cast(pl.Utf8) != pl.col(scimago_col).cast(pl.Utf8))
            ).height
            totals[col] += changes
            if changes > 0:
                print(df[col, scimago_col].filter(
                    pl.col(scimago_col).is_not_null()
                    & (pl.col(col).cast(pl.Utf8) != pl.col(scimago_col).cast(pl.Utf8))
                ))
                print(f"  - '{col}' updates: {changes}")
            df = df.with_columns(
                pl.when(pl.col(scimago_col).is_not_null())
                .then(pl.col(scimago_col))
                .otherwise(pl.col(col))
                .alias(col)
            )
...
def format_scimago_quartile_from_categories(categories_str: str | None, best_quartile: str | None) -> str | None:
    ...
    - Remove ' (miscellaneous)' from category names and strip trailing spaces.
...
def main():
    ...
    scimago_df = scimago_df.with_columns(
        ...
        # Preserve the raw best quartile as a separate column
    ...
    scimago_df = scimago_df.with_columns(
        pl.struct(["Categories_scimago", "Best Quartile_scimago"]).map_elements(
            lambda s: format_scimago_quartile_from_categories(s.get("Categories_scimago"), s.get("Best Quartile_scimago")),
